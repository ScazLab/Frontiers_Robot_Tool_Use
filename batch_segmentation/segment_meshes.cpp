		...
		try {
			...
			for (int k=K_START; k <= K_END; k += K_STEP) {			
				...
				for (float lambda=LAMBDA_START; lambda <= LAMBDA_END; lambda += LAMBDA_STEP) {			
                    ...
                    std::ifstream ifile;
                    std::string mesh_outout_filepath = get_filepath_from_k_and_lambda(OUTPUT_DIR + mesh_filename, k, lambda);
                    mesh_outout_filepath = change_filename_ext(mesh_outout_filepath, "ply");
                    ifile.open(mesh_outout_filepath);
                    if (ifile) {
                    	ifile.close();
                    	if (VERBOSE) std::cout << "already segment: " << mesh_outout_filepath << std::endl;
                    	continue;
                    }
			...
			std::cout << std::endl;
		} catch (...) { // unfortunately, this won't catch floating point error, as this is not an exception
